...
public class TableInfo implements Constants {
    ...
    private String formatLogicDeleteSql(boolean isWhere) {
        final String value = isWhere ? logicDeleteFieldInfo.getLogicNotDeleteValue() :
                logicDeleteFieldInfo.getLogicDeleteValue();
        if (isWhere) {
    ...
    void setFieldList(List<TableFieldInfo> fieldList) {
        ...
        /* 校验字段合法性 */
        Assert.isTrue(logicDeleted.get() <= 1, "@TableLogic not support more than one in Class: \"%s\"",
                entityType.getName());
