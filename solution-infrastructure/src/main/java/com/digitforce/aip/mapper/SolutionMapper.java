...
public interface SolutionMapper extends BaseMapper<Solution> {
    @Update("UPDATE solution SET status = '${status}' WHERE task_id = #{taskId} AND status != 'ONLINE' AND deleted = 0")
    int updateStatusByTaskId(@Param("taskId") Long taskId, @Param("status") SolutionStatusEnum status);
